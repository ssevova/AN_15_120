...
void plotAllHadrPresel()
{
  ...
  bool blind=false; int masksamples = 0x0111111; //0x0010000; //Default 0x1111111
  ...
  bool SaveGenLevelTMVA=true; //if true will be used only for ttbar semileptonic
  ...
  // string inputdir = "baconbits091015_allhadronic_njets05more3_tightmuo_04prod_3"; //trigger wrong
  string inputdir = "baconbits091015_allhadronic_njets05more3_tightmuo_04prod_4"; 


  string inputdirQCD = inputdir;


  string outputDir("semilep_1tightmuo_resolved_3ormorejets_2ormorejetWPm_pfmetmore100_pfmtmore40_trigrequestonMC_17102015");


  //  string outputDir("semilep_1tightmuo_resolved_3ormorejets_1ormorejetWPm_pfmetmore40_pfmtmore40_notrigrequestonMC_14102015");
  //  string outputDir("semilep_1tightmuo_resolved_3ormorejets_1ormorejetWPm_pfmetmore100_pfmtmore40_notrigrequestonMC_14102015");

  //  string outputDir("semilep_1tightmuo_resolved_3ormorejets_1ormorejetWPm_pfmetmore100_pfmtmore40_notrigrequestonMC_mvacut0_14102015");
  //  string outputDir("semilep_1tightmuo_resolved_3ormorejets_1ormorejetWPm_pfmetmore100_pfmtmore40_notrigrequestonMC_mvacut0_righthadrtops_semilepttbaronly_14102015"); 
  //string outputDir("semilep_1tightmuo_resolved_3ormorejets_1ormorejetWPm_pfmetmore100_pfmtmore40_notrigrequestonMC_mvacut0_notrighthadrtops_semilepttbaronly_14102015"); 
  
//  string outputDir("semilep_1tightmuo_resolved_3ormorejets_1ormorejetWPm_pfmetmore100_pfmtmore40_notrigrequestonMC_righthadrtops_semilepttbaronly_14102015");
  //  string outputDir("semilep_1tightmuo_resolved_3ormorejets_1ormorejetWPm_pfmetmore100_pfmtmore40_notrigrequestonMC_notrighthadrtops_semilepttbaronly_14102015");
 ...
 if(!phosel){
   ...
   //Modify samples to be investigated

   //MET trigger studies moved in DefineINputSamples
   /*   if(singlelepdata){
     cout<<"samplev.size() "<<samplev.size()<<endl;
     int issdata = -999;
     //looking for position of data sample (assuming only one)
     for(int iss=0; iss<samplev.size(); iss++){
       cout<<"iss "<<iss<<endl;
       if((samplev[iss]->label).compare("data")==0){
     ...
     if(issdata>=0){
       vector<CSample*> tmp;
       tmp.push_back(new CSample("data",0,0));
       tmp.back()->fnamev.push_back("../"+inputdir+"/SingleMu_2012-22Jan2013_allhadrbits.root"); 
       //tmp.back()->fnamev.push_back("../"+inputdir+"/SingleElectron_2012-22Jan2013_allhadrbits.root");
       samplev.insert(samplev.begin()+issdata,tmp.begin(),tmp.end());
       }
     cout<<"issdata "<<issdata<<endl;
 ...
 const string puWeightFilename("/tthome/ksung/cms/Analysis/03/CMSSW_5_3_14_patch2/src/DMSAna/Utils/data/PUWeights_2012.root");
...
if(verbose==2)
 ...
 //
  ...
  unsigned int njets05, nfjets, /* njets08,*/ nbjets, nbljets;         // jet multiplicity
   ...
   for(unsigned int ifile=0; ifile<sample->fnamev.size(); ifile++) {
    ...
    intree->SetBranchAddress("cutflow",    &cutflow);
	 ...
	 if(njets05<5){
       ...
       //       cout<<"BBB njets05 "<<njets05<<" res_jet "<<res_jet1<<", "<<res_jet2<<", "<<res_jet3<<", "<<res_jet4<<", "<<res_jet5<<", "<<res_jet6<<", "<<endl;
	 ...
	 if(res_jet3csv>BTAG_WPL  && fabs(res_jet3->Eta())<2.4)
       ...
       //	 cout<<"qui5 njets05 "<<njets05<<" "<<res_jet4<<" "<<res_jet5<<" "<<res_jet6<<endl;
     if(njets05>3){
       if(res_jet4 != 0){
     ...
     if(res_jet1 != 0 && res_jet2 !=0 && njets05>=2)
     ...
     //       wmass = Calcwmass(res_jet1,res_jet2,res_jet3,res_jet4,res_jet5,res_jet6); //to be done
...
//       else continue;
       ...
       //if(njets05  < 4)  continue;	
       if(njets05  < 3)  continue;	
       ...
       if(pfmt<40) continue;
       //       if(isHadrTopHighestMVA!=0) continue;
       //if(res_topmva<0) continue;	
       ...
       if(verbose)
	 cout<<"Event Passed selection"<<endl;



       TLorentzVector multijet = (*res_jet1)+(*res_jet2)+(*res_jet3);
       if(res_jet4 != 0)
	 multijet += (*res_jet4);
       if(res_jet5 != 0)
	 multijet += (*res_jet5);
	 ...
	 cout<<"Define variables to be plotted"<<endl;
       ...
       listvar.push_back(njets05); listvar.push_back(nbjets); listvar.push_back(nbljets_mt);
       ...
       if(njets05>0 && res_jet1 != 0)
	 listvar.push_back(res_jet1->Eta() && res_jet1->Pt() != 0); 
       else
       	 listvar.push_back(-999.);
       if(njets05>1 && res_jet2 != 0 && res_jet2->Pt() != 0)
	 listvar.push_back(res_jet2->Eta()); 
       else
	 listvar.push_back(-999.);       
       if(njets05>2 && res_jet3 != 0 && res_jet3->Pt() != 0)
	 listvar.push_back(res_jet3->Eta());
       else
	 listvar.push_back(-999.);
       if(njets05>3 && res_jet4 != 0 && res_jet4->Pt() != 0) 
       	 listvar.push_back(res_jet4->Eta());
       else
       	 listvar.push_back(-999.);
       if(njets05>4 && res_jet5 != 0 && res_jet5->Pt() != 0) 
       	 listvar.push_back(res_jet5->Eta());
       else
       	 listvar.push_back(-999.);
       if(njets05>5 && res_jet6 != 0 && res_jet6->Pt() != 0) 
